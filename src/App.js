...
class App extends Component {
  ...
  searchResultsArray = () => {
    const locationMarkers = this.state.queryList
      .map((item, index) => {
        return (
          item.displayString +
          (this.state.middleLocation.includes(index) ? "|marker-green" : "")
        );
      })
      .join("||");
                ...
                <div className="search-list col-50">
                  {!this.state.queryList.length > 0 ? (
                    <p className="main-intro">
                      Welcome to Shopper Mapper - for when you want to go
                      somewhere average. Enter your location and what you’re
                      looking for (e.g. museum, restaurant, cafe) and we’ll list
                      options within 10km, highlighting what’s not too good, not
                      too bad, but right in the middle.
                    </p>
                  ) : null}
                  {this.state.loading ? (
                  ...
                  {this.state.directionsArr ? (
                    <Directions
                      ...
                      directionsArray={this.state.directions}
                    />
                  ) : null}
                </div>
